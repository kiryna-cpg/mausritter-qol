...
function isItemPilesActor(actor) {
  try {
    // Heurística 1: tipo actor (si tu sistema usa tipo "pile")
    if (actor?.type && String(actor.type).toLowerCase().includes("pile")) return true;

    // Heurística 2: flags (pero validando "enabled" o data.itemPile)
    const f = actor?.flags?.["item-piles"];
    if (!f) return false;

    // Si el módulo deja un boolean "enabled" o "itemPile"
    if (f.enabled === true) return true;
    if (f.itemPile === true) return true;

    // Si deja data con alguna marca típica
