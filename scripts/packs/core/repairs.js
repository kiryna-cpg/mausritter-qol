...
export async function repairItem(item, amount) {
  ...
  if (!quote?.ok) {
    ...
    const key = reasonToKey[quote?.reason] ?? "MRQOL.Repairs.NothingToRepair";
    if (quote?.reason === "noActor") ui.notifications.warn(game.i18n.localize(key));
  ...
  if (quote.currency < quote.total) {
    return ui.notifications.warn(
      game.i18n.format("MRQOL.Repairs.NotEnoughPips", { need: quote.total, have: quote.currency })
    );
  }

  await actor.update({ [PATHS.actorCurrency]: quote.currency - quote.total });
  await item.update({ [PATHS.itemPipsValue]: quote.used - quote.toRepair });

  ui.notifications.info(
    game.i18n.format("MRQOL.Repairs.Done", { spent: quote.total, pips: quote.toRepair })
  );
}