...
// - Rellena los huecos correctamente on dragdrop en todas las zonas. Ocupa casillas libres al borrar incluso en objetos de más de una casilla.
// 2. Badges en objetos apilados (más de un objeto en una casilla).
// - El badge numérico de objetos apilados aparece correctamente
// 3. Status Effect Encumbered
// - No activa automáticamente Encumbered cuando hay objetos apilados
// - Encumbered se puede activar manualmente y fuerza la tirada de salvaciones con ventaja/desventaja
// - Los objetos apilados fuerzan la ventaja/desventaja en las salvaciones sin activar Encumbered.
// 4. Incorrect Placement
// - No muestra el overlay de Incorrect placement
// 5. Equipar/Desequipar
// - Los botones de Equipar/Desequipar se muestran correctamente. Equipar/Desequipar funciona en Armor y Weapon, y Conditions.
// 6. Grit
// La zona de Grit se gestiona correctamente.
...
async function setEncumberedOnActiveTokens(actor, active) {
  try {
    // Linked + unlinked, include all active tokens in all scenes
    const tokens = actor?.getActiveTokens?.(true, true) ?? [];
    for (const t of tokens) {
      const doc = t?.document ?? t;
      if (typeof doc?.toggleStatusEffect === "function") {
        await doc.toggleStatusEffect(ENCUMBERED_ID, { active }).catch(() => {});
      }
    }
  } catch (_) {}
}
...
async function setActorEncumbered(actor, active) {
  ...
  const next = prev.finally(async () => {
    ...
    if (active) {
      ...
      // ✅ FIX 2: reflect on active tokens too
      await setEncumberedOnActiveTokens(actor, true);
    ...
    const idsNow = getEncumberedEffectIds(actor);
    if (!idsNow.length) {
      // ✅ FIX 2: ensure tokens get cleared too
      await setEncumberedOnActiveTokens(actor, false);
      return;
    }
    if (typeof actor.deleteEmbeddedDocuments !== "function") {
      await setEncumberedOnActiveTokens(actor, false);
...
function isEncumberedEffectively(actor) {
  ...
  // Manual status (ActiveEffect present)
  if (getEncumberedEffectIds(actor).length) return true;

  // Fallback: our own flag if AE isn't available
...
async function refreshEncumbered(actor) {
  ...
  const manual = getEncumberedEffectIds(actor).length > 0 || actor.getFlag?.(MODULE_ID, "encumbered");

  // Si hay manual ON, no lo apagues automáticamente
  if (manual && !auto) return;

  await setActorEncumbered(actor, auto);
}
...
if (!layout) {
  // If item is marked invalid, keep overlay (don't auto-clear)
  if (isItemIncorrectPlacement(item)) {
    // Ensure the overlay exists even after re-renders
    const msg = el.dataset.mrqolInvalidTitle || "Incorrect placement.";
    markInvalid(el, msg);
  } else {
    clearInvalidMark(el);
  }
