...
// Rellena los huecos correctamente on dragdrop en Pack, pero no en otras zonas y no ocupa casillas libres al borrar
// 2. Badges en objetos apilados (más de un objeto en una casilla)
// Los objetos apilados no muestran el badge numérico
...
// Los botones de Equipar/Desequipar no aparecen, o aparecen abajo en pequeño pero no funcionan
6. Grit
// No cuenta los espacios de Grit correctamente
...
if (pos && Number.isFinite(pos.x) && Number.isFinite(pos.y)) {
  ...
  // IMPORTANT: only compact after the update is committed
  if (actor?.documentName === "Actor") {
    await compactPackIfNeeded(actor).catch(() => {});
  }
}
...
function refreshToggleButtons(app, root, map) {
  ...
  for (const card of cards) {
    ...
    if (item.type === "condition") {
      let btn = controls.querySelector(".mrqol-grit-toggle");
      if (!btn) {
        btn = document.createElement("a");
        btn.className = "item-control mrqol-grit-toggle mrqol-toggle";
        btn.setAttribute("title", "Grit");
        btn.innerHTML = '<i class="fa-brands fa-brave"></i>';
        btn.addEventListener("click", (ev) => {
          ev.preventDefault();
          ev.stopPropagation();
