...
function _asTokenDocument(t) {
  if (!t) return null;
  if (t.documentName === "Token") return t;              // TokenDocument
  if (t.document?.documentName === "Token") return t.document; // Token placeable
  return null;
}

function getTokensForActor(actor) {
  const out = [];
  if (actor?.token) out.push(actor.token);
  try {
    const active = actor?.getActiveTokens?.(false, true) ?? [];
    for (const t of active) out.push(_asTokenDocument(t));
  } catch (_) {}
...
async function setEncumberedOnTokens(actor, active, tokenDoc = null) {
  ...
  for (const td of tokens) {
    if (!td) continue;
    const k = td.uuid ?? `${td?.parent?.id ?? td?.scene?.id ?? "?"}:${td.id ?? "?"}`;
    if (seen.has(k)) continue;
    seen.add(k);

    // Forzar estado visual del token
    await td.toggleStatusEffect?.(ENCUMBERED_ID, { active: isActive }).catch(() => {});

    // Y si hay TokenActor (synthetic/unlinked), sincronizarlo también
    const a = td.actor ?? null;
    if (!a || a === actor) continue;
    await a.toggleStatusEffect?.(ENCUMBERED_ID, { active: isActive }).catch(() => {});
  }
}
