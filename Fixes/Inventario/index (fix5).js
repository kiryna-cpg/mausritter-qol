...
// Los botones de Equipar/Desequipar aparecen al abrir la ficha pero desaparecen cuando se borran objetos o se reordenan automáticamente los objetos después de dejar una casilla libre. Equipar/Desequipar funciona en Armor y Weapon, y Conditions.
6. Grit
// No gestiona la zona Grit correctamente (coloca la Condition en el medio en lugar de en la casilla)
...
// ------------------------------------------------------------
// Inventory DOM observer (reinjet toggles on partial re-renders)
// ------------------------------------------------------------
const __mrqolInvObservers = new WeakMap();
const __mrqolInvObserverTimers = new WeakMap();

function ensureInventoryObserver(app, root) {
  const dragArea = root?.querySelector?.("#drag-area");
  if (!dragArea) return;

  if (__mrqolInvObservers.has(root)) return;

  const schedule = () => {
    if (__mrqolInvObserverTimers.has(root)) return;
    const t = setTimeout(() => {
      __mrqolInvObserverTimers.delete(root);
      try {
        // Root may have been replaced; re-resolve open sheets instead
        refreshInventoryUIForActor(app.actor).catch(() => {});
      } catch (_) {}
    }, 0);
    __mrqolInvObserverTimers.set(root, t);
  };

  const obs = new MutationObserver((mutations) => {
    // Any list rebuild / item card change => reinject
    for (const m of mutations) {
      if (m.type === "childList" && (m.addedNodes.length || m.removedNodes.length)) {
        schedule();
        return;
      }
      if (m.type === "attributes" && m.target?.dataset?.itemId) {
        schedule();
        return;
      }
    }
  ...
  __mrqolInvObservers.set(root, obs);

  // Safety: disconnect when sheet closes
  Hooks.once("closeActorSheet", (closingApp) => {
    if (closingApp !== app) return;
    try { obs.disconnect(); } catch (_) {}
    __mrqolInvObservers.delete(root);
    __mrqolInvObserverTimers.delete(root);
  });
}
...
Hooks.on("updateItem", (item, change) => {
  ...
  // Refresh UI
  afterTwoFrames(() => refreshInventoryUIForActor(actor).catch(() => {}));
