...
// - El overlay de Incorrect placement se muestra correctamente.
...
function getTokensForActor(actor) {
  ...
  // 1) TokenActor (unlinked/synthetic) suele tener actor.token (TokenDocument)
  const tok = actor?.token;
  if (tok) out.push(tok);

  // 2) Actor del world: tokens activos en escenas
  try {
    const active = actor?.getActiveTokens?.(false, true) ?? [];
    for (const t of active) out.push(t?.document ?? t);
  } catch (_) {}
...
async function setEncumberedOnTokens(actor, active, tokenDoc = null) {
  ...
  if (tokenDoc) tokens.unshift(tokenDoc);

  const seen = new Set();
  for (const doc of tokens) {
    const k = doc?.uuid ?? doc?.id;
...
async function setActorEncumbered(actor, active) {
  ...
  const next = prev.finally(async () => {
    ...
    // Persistimos el estado auto en flag (fallback/debug)
    await actor.setFlag(MODULE_ID, "encumberedAuto", isActive).catch(() => {});

    // ✅ Fuente de verdad VISUAL: tokens
    await setEncumberedOnTokens(actor, isActive);
  });
...
function enforceEncumberedOnTokenUpdate(tokenDoc) {
  try {
    ...
    // fuerza que el token “real” se sincronice sí o sí
